...
class Mailer extends \Illuminate\Mail\Mailer
{
    ...
    public function send($view, array $data = [], $callback = null)
    {
        ...
        $symfonyMessage = clone $message->getSymfonyMessage();

        if (method_exists($message, 'getBcc') && $message->getBcc()) {
            foreach ($message->getBcc() as $bcc) {
                if (method_exists($symfonyMessage, 'addBcc')) {
                    $symfonyMessage->addBcc($bcc->getAddress(), $bcc->getName());
                } elseif (method_exists($symfonyMessage, 'bcc')) {
                    $symfonyMessage->bcc($bcc->getAddress(), $bcc->getName());
                }
            }
        }
